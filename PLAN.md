...
### ✅ M2 — Login online-mode (chiffrement) + compression — **TERMINÉ** (commit fa99c75)
**Atteint** : un compte premium rejoint en online-mode — Encryption Request (avec *Should Authenticate*) → Encryption Response (RSA/PKCS#1v1.5) → chiffrement AES/CFB8 actif → `hasJoined` Mojang → profil authentifié (vrai UUID + properties signées) → Set Compression (frontière exacte avant Login Success) → zlib actif. Vérifié in vivo par le user (vrai compte officiel, `mode=online`). La skin en jeu dépend du paquet **Player Info Update** (état Play) qui arrive en M4 — M2 récupère et forwarde bien `textures` (dans Login Success), c'est l'affichage qui attend M4. Briques `frame` (CFB8/zlib) et `auth` (RSA/DER/hash/hasJoined) déjà faites en M0 ; M2 = câblage du flux + 3 paquets. Tests niveaux 1 & 2 verts (golden-bytes + intégration scriptée chiffrement+compression de bout en bout).
